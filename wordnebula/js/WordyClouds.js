...
function createWords(text, commonWords){
	...
	word_dict[words[0]] = new Word(words[0])			//generate the first word in the list. Need to deal with corner case of empty
	for (var i=0; i+1<words.length; i++){
		if (words[i+1] in word_dict)					// checks to see if the word is in the dictionary, i.e. has it been seen before?
			word_dict[words[i+1]].freq++				// if it has been seen, increase the number of times the word has been seen by 1.
		else											//creat the word object and add it to the dictionary of words
			word_dict[words[i+1]] = new Word(words[i+1])
...
Layouts.printWords = function PrintWordsLayout(canvas, colors, fontName){
    ...
    var words = keys(word_dictionary).sort(function(one, other){
        return word_dictionary[other] - word_dictionary[one]
    })
	//THIS MAY NEED TO BE MODIFIED TO ACCOMODATE THE STRUCTURE OF EACH WORD OBJECT
	...
	words.forEach(function(word){
		...
		if( (canvas.width - avoid_x) > textWidth){
			var x = avoid_x;
			var y = avoid_y;
		...
		else if((canvas.width - avoid_x) < textWidth){
			avoid_x = 20;
			var x = avoid_x;
			avoid_y += textHeight+pad;
			var y = avoid_y;	
			//console.log('avoid: ' + x + ', ' + y)			
			//var c = Math.floor(Math.random() * colors.length)
        	//var c = colors[c]
...
WordyClouds.loadFromWordFreq = function(){
    ...
    var palate = ColorPalates.scheme1
...
WordyClouds.processClick = function(x, y){
	...
	for (var word in word_dictionary){
		...
		word_dictionary[word].color = 'grey';
	}
	for (var word in word_dictionary){
		var wordObj = word_dictionary[word];
		if (x >= wordObj.x && x <= (wordObj.x + wordObj.width) && y <= wordObj.y && y >= (wordObj.y - wordObj.height)){
			...
			word_dictionary[wordObj.text].color = 'rgba(255, 0, 0, 0.5)';
			...
			for (var i = 0; i < list.length; i++){
				...
				word_dictionary[list[i]].color= 'rgba(0, 0, 255, '+col+')';
